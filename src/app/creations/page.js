...
export default function CreationsPage() {
  ...
  return <main className="workspace-page">
    ...
    <Modal.Backdrop isOpen={!!selected} onOpenChange={(open) => { if (!open) setSelected(null); }}><Modal.Container size="lg"><Modal.Dialog className="result-dialog"><Modal.CloseTrigger /><Modal.Body>{selected && <div className="result-grid">{selected.image ? <Image src={selected.image} alt={selected.prompt} width={800} height={800} unoptimized /> : <div className="result-state"><CreationState item={selected} /></div>}<ResultInfo eyebrow="CREATION DETAILS" prompt={selected.prompt} model={selected.model} ratio={selected.ratio} createdAt={selected.createdAt} actions={<><Button variant="danger" onPress={() => remove(selected.id)}><TrashIcon /> Delete</Button><Button variant="outline" slot="close">Close</Button></>} /></div>}</Modal.Body></Modal.Dialog></Modal.Container></Modal.Backdrop>
  </main>;
}
