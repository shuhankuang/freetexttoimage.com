...
export default function CreationsPage() {
  ...
  return <main className="workspace-page">
    ...
    {items.length > 0 && <><div className="library-summary"><span><strong>{items.length}</strong> creations</span><span>Saved to your account</span></div><div className="creation-grid">{items.map((item) => <Card key={item.id} className="creation-card"><CreationImage item={item} onClick={() => setSelected(item)} /><Card.Content><div><span>{[item.model, item.ratio].filter(Boolean).join(" · ") || "AI image"}</span><time>{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(item.createdAt))}</time></div><Card.Title>{item.title}</Card.Title><Card.Description>{item.prompt}</Card.Description></Card.Content><Card.Footer><Button size="sm" variant="ghost" onPress={() => setSelected(item)}>View details <ArrowIcon /></Button></Card.Footer></Card>)}</div></>}
    ...
    <Modal.Backdrop isOpen={!!selected} onOpenChange={(open) => { if (!open) setSelected(null); }}><Modal.Container size="lg"><Modal.Dialog className="result-dialog"><Modal.CloseTrigger /><Modal.Body>{selected && <div className="result-grid">{selected.image ? <Image src={selected.image} alt={selected.title} width={800} height={800} unoptimized /> : <div className="result-state"><CreationState item={selected} /></div>}<ResultInfo eyebrow="CREATION DETAILS" title={selected.title} prompt={selected.prompt} model={selected.model} ratio={selected.ratio} createdAt={selected.createdAt} actions={<><Button variant="danger" onPress={() => remove(selected.id)}><TrashIcon /> Delete</Button><Button variant="outline" slot="close">Close</Button></>} /></div>}</Modal.Body></Modal.Dialog></Modal.Container></Modal.Backdrop>
  </main>;
}
